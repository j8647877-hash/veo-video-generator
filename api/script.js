...
// Also handles ?action=social for social-media copy generation.
// Uses the user's Google OAuth token (same pattern as the image/video tools).
module.exports = async function handler(req, res) {
  // Route to social handler when ?action=social
  if (req.query?.action === 'social') return handleSocial(req, res);
...
// ── Social media copy generation (POST ?action=social) ──────────────────────
async function handleSocial(req, res) {
  ...
  let userMessage = `Write a social media post for ${platform}.\n`;
  userMessage += `Tone: ${tone}\n\n`;
  userMessage += `Topic / Instructions:\n${prompt.trim()}\n\n`;

  if (platform === 'Twitter' || platform === 'X') {
    userMessage += `Constraint: Keep it concise and within Twitter's character limits (under 280 characters if possible, or format as a thread if instructed).\n`;
  } else if (platform === 'LinkedIn') {
    userMessage += `Constraint: Format for LinkedIn with good spacing, professional insights, and an engaging hook.\n`;
  } else if (platform === 'Instagram' || platform === 'TikTok') {
    userMessage += `Constraint: This is for a highly visual platform. Make the caption engaging, punchy, and include a strong hook.\n`;
  }

  if (includeHashtags) {
    userMessage += `\nResearch and include 3-5 of the top trending and highly relevant hashtags based on the topic and niche.\n`;
  }
